...
function branchesValidation(data) {
  const branchesSchema = Joi.object({
    name: Joi.string().min(2).max(15).pattern(/^[a-zA-Z]+$/).required(),
    ...
    address: Joi.string().custom((value, helpers) => {
      ...
      "string.invalidFormat": "Invalid address format, Only letters, spaces, and ['.-] are allowd."
    }),
...
function branchesValidationUpdate(data) {
  const branchesSchema = Joi.object({
    name: Joi.string().min(2).max(15).pattern(/^[a-zA-Z]+$/),
    image: Joi.string(),
